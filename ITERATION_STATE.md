...
### 设计稿缺失可见性（2026-09-06）
- **需求**：看板多卡片显示「请上传设计稿」占位但无标记无筛选，缺稿不醒目
- **实现**：判定 = 任务 join 后 pdf_url 为空（即款式未上传设计稿）；看板卡片左上角橙色「缺设计稿」角标（右上角逾期徽章不受影响）；筛选区新增橙色「全部设计稿/缺设计稿/已有设计稿」下拉（与现有 5 个筛选并列，看板/列表共用 filterTasks）
- **验证**：构建通过；数据实测 8 任务中缺稿 6（235-6/JK001×2/TS003×2/PA004）、有稿 2（26AWW526/DR002），与看板样本逐卡一致；浏览器工具本会话沙箱不可用，待用户页面确认
