...
- **U19 渲染性能（`4206a7b`）**：TaskCard 包 `React.memo`（onTaskClick 走 useCallback 稳定引用），KanbanView 的 filteredTasks / activeCols / groupedTasks 转 `useMemo`；`App.jsx` 的 handleStatusSync 由「全量 loadTasks()」改单条 `GET /api/tasks/:id` 就地合并（保存后不再整表重载）。
- **U20 缩略图懒渲染（`820e6d5`，修订 `508d6d5`）**：新增 `src/utils/thumbQueue.js`（并发≤2；cancel 未启动分支 splice 移除 + pump + resolve(null)）；`PdfThumb` 接 IntersectionObserver 首次入视口才渲染、卸载解绑并 cancel；`pdf.js` 的 renderPdfThumb 失败静默返回 null（去掉 console.error）。**主理人打回**：失败占位点击无反应（放大层无入口）、非卡片场景 `.pdf-empty` 退化为裸文本块 → 508d6d5 给失败态补 interactiveProps（放大层「更换设计稿 / 从资料库选」可达）+ 补 `.pdf-empty` 全局基样式；新增 `tests/utils/thumbQueue.test.js`（3 用例，单测 95→98）。
- **U21 SmartSelect 事件驱动定位（`8720a1e`，修订 `13d900b`）**：删除每帧 rAF 轮询，改为打开时绑 scroll(捕获)/resize/ResizeObserver + rAF 节流。**主理人打回**：鼠标点开下拉（焦点留在 overlay）按 Esc 全无反应（死锁：Esc 只在 `.ss-dropdown` 内 onKeyDown，而 Modal 文档捕获无条件让位）→ 13d900b 在 open 期间于 document 捕获阶段绑 keydown（Esc → stopPropagation + 关闭），焦点在下拉内时无回归。
- **U22 响应式三断点（`0e52a2d`，修订 `8e2adf4`）**：`views.css` 尾部新增 1366/1200/1024 断点（`--board-col-w` 600→420→380→360）。**主理人打回**：1024 下列宽 300px 致卡片内部元素级溢出（4/6 卡）与节点文字重叠 → 8e2adf4 列宽下限抬至 380/360 + `.bento-style-no` 的 `overflow/text-overflow/min-width` 由「仅逾期卡」改为全局生效。
- **U22 补修 · 窄宽度逾期卡款号截断（`0757662`）**：现象「SS…」。**量化根因**：右栏宽 = 列宽 − 190；逾期卡 `.bento-tr` 另留 104px 给绝对定位徽标，与 40px 行高的款号同处一条色带争宽 → 款号可用宽 1366=106px / 1200=66px / 1024=46px，而 24px「AW26-JK001」需 149px（1024 下**非逾期**卡 134px 亦不足）。**修法**：徽标 DOM 由卡片级移入 `.bento-tr` 并作首个子元素（桌面态 `.bento-tr` 无 position → 徽标仍 absolute、定位祖先仍是 `.bento-card` → 1440 零漂移）；≤1366 该行改 `flex-direction:column` + 徽标 `position:static` 独占一行、款号整行可用，并以 `.bento-right-col{gap:8px}` 抵消行增高；≤1200/≤1024 款号字号降 20/18px。未改徽标 background/color/box-shadow/z-index:8，today/soon 内联样式保留。
- **主理人独立复验（自建探针，不复用工程师脚本；真机 Chrome CDP 1440/1366/1200/1024 × 6 卡 = 24/24 全绿）**：款号 `scrollWidth==clientWidth`（截断归零）、逾期徽标 ∩ 款号 = 0、徽标 ∩ 设计师徽标 = 0、徽标全在卡内、卡片高 238±8（实测 238~246，与基线 `_probe6.json` 的 {238,246} 同包络）、元素级溢出 0、节点重叠 0、`documentElement.scrollWidth==innerWidth`、consoleErrors=0。**1440 零漂移**：与基线 `_probe7.json` 逐卡款号宽度完全相同（149/149/119/75）、徽标仍 absolute（top/right 10px，含 1px 卡片边框 → 实测内缩 11/11）、卡高 238。**交互复核**：真实鼠标 hover → 次级浮层 opacity 0→1 正常浮出、徽标 hover 前后几何不变且 z-index 8 压在浮层(z=6)之上、`.bento-tr` overflow:visible 无裁切、移出即收起。
- **门禁（提交态复跑）**：`eslint src` 0 error / `doc:check` STATS 一致 / `npm test` 98-98（含 rollback 20/20、上传安全 43+ 项）/ `vite build` 11.41s 全绿。
- **批4 完成状态**：U13 → U17 → U14 → U15 → U16 → U18 → U19 → U20 → U21 → U22 全部落地并推送；远端 `feature/sample-run-model` = `0757662`（本地 = 远端）。
- **环境教训（本轮新增）**：①dev server 与 API 只监听 `[::1]`，端口探测别用 `127.0.0.1`（会假报 CLOSED）；②`_chrome_lead*` 探针 profile 若留在项目根会被 Vite 依赖扫描吞进（报 `Failed to run dependency scan … chrome not resolved`），探针 profile 一律放系统临时目录；③后台 `(cmd &)` 子壳会被回收，长驻服务须用「后台任务承载前台进程」；④PowerShell 通道本会话 stdout 被完全吞（只回 exit code），清僵尸进程用 `MSYS_NO_PATHCONV=1 taskkill /F /T /PID`。
