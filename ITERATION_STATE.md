...
## 批11 交接记录：REQ-037/038 视觉"干净"未收口（2026-09-16 晚，用户移交他人）
- **结论（如实，2026-09-16 晚用户更正）**：REQ-037/038 数据穿透**未修复**——用户实测仍穿透（"数据穿透技术修复无效"），此前"技术修复有效（批6-10）"的结论不成立，予以推翻；同时设置页视觉"干净"也未获验收。用户最终评价"一个晚上什么问题都没解决"，指示将未解决问题标记移交他人处理。
- **用户验收史（重要）**：批9 误以为收口被用户两轮打回（"不是一个界面""抄都不会"）；批10 按参考形态收口（表头同容器色、行胶囊化、公差纯文字）仍被否；批11 我擅改结构（表格→列表）被用户严厉否决——用户从最初就立规矩"只改视觉层，不碰 JSX 结构/布局/列顺序/交互/数据流"（实施方案 5.1 硬边界），我违反了。已 git checkout 还原全部结构改动。
- **用户最后明确的整改方向（接手者必读）**："人家界面干净是没那么多线框"——参考原型（ui_ref/src/components/SettingsView.tsx）干净的根源是线框少（表头无底线、行无分隔网格线、选中分类无边框、标签无边框），结构保留的前提下做去线框。
- **当前工作树**（仅 components.css，+3/-3，未提交）：①th 去 border-bottom（表头底线）；②.badge-req 去 1px 红边框；③.cat-item.active 去 border-color（保留 accent-soft 弱底）。这三处是用户"去线框"方向的直接响应，已保留待提交。
- **未决事项（移交清单）**：
  1. **REQ-037/038 数据穿透未修复（用户实测无效）**——批6-10 的"修复"未通过用户实测，接手者需先复现穿透场景（sticky 表头/冻结列滚动时行内容透出），再做根因级修复，不能沿用 CDP 自测即判定修复的旧做法。
  2. 批11 已还原，若用户认可"去线框"方向，按当前 3 行 CSS 思路推进；若不认可，需与用户重新对齐"干净"的具体定义（建议先让用户圈出具体仍显"框"的元素）。
  3. 推送需代理：本地 feature/next-milestone 领先远端 8 提交（批6-10），未推送。
- **参考权威**：ui_ref/src/components/SettingsView.tsx（卡片/列表/去线框形态原文）、ui_ref/src/index.css（token）；实施方案飞书 Vzh1...（含 5.1 硬边界）；项目摘要飞书 RGRj...（REQ-037/038 行）。