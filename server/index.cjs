...
// 白名单值集中在 config.cjs（G17）：未来拆分服务器端时在此调整唯一来源。
...
const isAllowedOrigin = (origin, callback) => {
  ...
  try {
    ...
    const allowed =
      u.protocol === 'file:' ||
      (config.ALLOWED_HOSTS.includes(u.hostname) && config.ALLOWED_PORTS.includes(u.port));
...
// ── 鉴权前置中间件占位（G17 · 前后端拆分"留缝"）─────────────────────────────
// 未来接入用户认证 / 拆分服务器端时，**只需改这一处**：
//   在此校验凭证（如 Authorization / Session Cookie），未通过则 `res.status(401).end()`。
// 现为 no-op（完全放行），保证单机版行为与其他中间件顺序不变。
// 位置：位于 CORS 之后、body 解析之前 —— 未授权请求在写路径之前即被拦截，且 CORS 头已就绪。
app.use((req, res, next) => next());

app.use(express.json({ limit: config.JSON_BODY_LIMIT })); // 支持 base64 大文件（含 dxf 等专业格式）
...
const LOG_BODY_MAX_BYTES = config.LOG_BODY_MAX_BYTES; // 超过此体积不序列化 body，仅记 [body omitted]
...
function startServer(overridePort, dbPath, uploadsPath) {
  ...
  // 静态文件服务
  app.use(config.UPLOADS_MOUNT_PATH, express.static(getUploadsDir()));

  // 处理打包后的静态资源路径
  const DIST_DIR = path.join(__dirname, config.DIST_DIR_REL);
  if (fs.existsSync(DIST_DIR)) {
    app.use(express.static(DIST_DIR));
    app.get(/(.*)/, (req, res, next) => {
      if (!req.path.startsWith(config.API_PATH_PREFIX) && !req.path.startsWith(config.UPLOADS_PATH_PREFIX)) {
